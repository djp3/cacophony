...
public class InstanceQuery 
  extends DatabaseUtils implements OptionHandler,Serializable {
  ...
  public Instances retrieveInstances() throws Exception {
	...
	public Instances retrieveInstances(String query) throws Exception {
		...
		try {
			...
			// For sqlite
			// cache column names because the last while(rs.next()) { iteration
			// for
				...
				for (int i = 1; i <= numAttributes; i++) {
					...
					case STRING:
						...
						if (rs.wasNull()) {
							...
							Double index = (Double) nominalIndexes[i - 1]
									.get(str);
							if (index == null) {
			...
			for (int i = 0; i < numAttributes; i++) {
				...
				case Attribute.STRING:
					...
					for (int n = 0; n < nominalStrings[i].size(); n++) {
						att.addStringValue((String) nominalStrings[i]
								.elementAt(n));
					}
					break;
				case Attribute.DATE:
			...
			result = new Instances("QueryResult", attribInfo, instances.size());
