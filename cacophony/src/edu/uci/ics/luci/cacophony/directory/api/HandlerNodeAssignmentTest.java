...
public class HandlerNodeAssignmentTest {
	...
	public void testGetNodeAssignment() {
		...
		try {
			...
			responseString = WebUtil.fetchWebPage("http://localhost:" + testPort + "/node_assignment", false, params, 30 * 1000);
		} catch (MalformedURLException e) {
			e.printStackTrace();
			fail("Bad URL:"+e);
		} catch (IOException e) {
		...
		try {
			...
			try {
				...
				assertTrue(Globals.getGlobals().getVersion(),response.getString("node_id") != null);
				assertTrue(Globals.getGlobals().getVersion(),response.getString("node_configuration") != null);
			} catch (JSONException e) {
				e.printStackTrace();
