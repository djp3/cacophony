...
public class Test {
	...
	private static void dberrprint(DB db, String func) {
		Error err = db.error();
		printf("%s: %s: %d: %s: %s\n", Test.class.getName(), func, err.code(),
	...
	private static int procorder(String path, long rnum, int thnum,
			boolean rnd, boolean etc) {
		...
		class Setter extends Thread {
			...
			public void run() {
				...
				for (long i = 1; !err_ && i <= rnum_; i++) {
					String key = String.format("%08d", rnd_ ? rand(range) + 1
							: base + i);
					if (!db_.set(key, key)) {
		...
		Setter.setDefaultUncaughtExceptionHandler(EXH);
		...
		if (etc) {
			...
			class Adder extends Thread {
				...
				public void run() {
					...
					long range = rnum_ * thnum_;
					for (long i = 1; !err_ && i <= rnum_; i++) {
						String key = String.format("%08d",
								rnd_ ? rand(range) + 1 : base + i);
						if (!db_.add(key, key)
								&& db_.error().code() != Error.DUPREC) {
			...
			Adder.setDefaultUncaughtExceptionHandler(EXH);
			for (int i = 0; i < thnum; i++) {
		...
		if (etc) {
			...
			Appender.setDefaultUncaughtExceptionHandler(EXH);
			for (int i = 0; i < thnum; i++) {
		...
		if (etc) {
			...
			Accepter.setDefaultUncaughtExceptionHandler(EXH);
			for (int i = 0; i < thnum; i++) {
		...
		Getter.setDefaultUncaughtExceptionHandler(EXH);
		for (int i = 0; i < thnum; i++) {
		...
		if (etc) {
			...
			class InnerTraverser extends Thread {
				...
				public InnerTraverser(int id, long rnum, int thnum,
						boolean rnd, DB db) {
					id_ = id;
					rnum_ = rnum;
					rnd_ = rnd;
					...
					class VisitorImpl implements Visitor {
						...
						public byte[] visit_full(byte[] key, byte[] value) {
							...
							if (rnd_) {
								switch ((int) rand(7)) {
								case 0: {
									rv = String.format("%d%d", cnt_, cnt_)
											.getBytes();
									break;
								}
								case 1: {
			...
			InnerTraverser.setDefaultUncaughtExceptionHandler(EXH);
			for (int i = 0; i < thnum; i++) {
		...
		if (etc) {
			...
			OuterTraverser.setDefaultUncaughtExceptionHandler(EXH);
			for (int i = 0; i < thnum; i++) {
		...
		Remover.setDefaultUncaughtExceptionHandler(EXH);
		for (int i = 0; i < thnum; i++) {
	...
	private static int procwicked(String path, long rnum, int thnum, int itnum) {
		...
		for (int itcnt = 1; itcnt <= itnum; itcnt++) {
					...
					for (long i = 1; !err_ && i <= rnum_; i++) {
						...
						case 4: {
							if (rand(2) == 0) {
								...
								if (Double.isNaN(db_.increment_double(key,
										rand(1000) / 100.0, 0))
										&& db_.error().code() != Error.LOGIC) {
						...
						case 8: {
							...
							if (rand(rnum_ / 50 + 1) == 0) {
								String origin = key.substring(0,
										key.length() - 1);
								if (db_.match_similar(origin, 3, rand(2) == 0,
										rand(10)) == null) {
			...
			Operator.setDefaultUncaughtExceptionHandler(EXH);
			for (int i = 0; i < thnum; i++) {
	...
	private static int procmisc(String path) {
		...
		try {
			...
			if (db.get("tako") != null || db.get("kani") != null
					|| db.count() != cnt) {
			...
			if (suffix != null) {
				...
				for (int i = 0; i < copypaths.length; i++) {
					copypaths[i] = String.format("%s.%d%s", corepath, i + 1,
							suffix);
				}
	...
	private static int procmemsize(long rnum, String path) {
		...
		long count = -1;
		if (path == null) {
			int bnum = rnum < Integer.MAX_VALUE ? (int) rnum
					: Integer.MAX_VALUE;
